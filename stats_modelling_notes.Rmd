...
importance(neighbourhood_rf_classifier)
```
...
```{r}
#Impute the test dataset using the same imputation method as before
...
nrating_regression_preds <- train_nrating_regression %>% 
  ...
  add_predictions(mod_nrating, type = "response") %>% 
...
roc_obj_1_term <- mortgage_data_with_1pred %>% 
  roc(response = accepted, predictor = pred)#predictor is the probability column

ggroc(data = list(one_term = roc_obj_1_term, two_term = roc_obj_3_terms), legacy.axes = TRUE)+
  labs(x = "False Positive Rate", y = "True positive rate")
...
ggroc(list(random_forest = roc_obj_nrating_tree, log_regression = roc_obj_nrating_regression_test))+
  labs(x = "False Positive Rate", y = "True positive rate")
